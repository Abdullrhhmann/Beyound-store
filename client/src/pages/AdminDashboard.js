...
const AdminDashboard = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch('http://192.168.43.103:5000/api/orders');
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      const response = await fetch(`http://192.168.43.103:5000/api/orders/${orderId}`, {
        method: 'PUT',
  ...
  const updateProductPrice = async (productId, newPrice) => {
    try {
      const response = await fetch(`http://192.168.43.103:5000/api/products/${productId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
